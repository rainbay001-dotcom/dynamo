...
impl OpenAIPreprocessor {
    ...
    pub async fn gather_multi_modal_data<R: OAIChatLikeRequest>(
        ...
        let mut fetch_tasks: Vec<(String, usize, &ChatCompletionRequestUserMessageContentPart)> =
            Vec::new();
                ...
                match (url, uuid) {
                    (Some(url_val), _) => {
                        if has_media_loader {
                            ...
                            #[cfg(feature = "lightseek-mm")]
                            if type_str == "image_url" {
                                let mm_hash = Self::hash_image_url(url_val.as_str());
                                url_passthrough_images.push((mm_hash, url_val.to_string()));
                            }
                            media_map
                                .entry(type_str.to_string())
                                .or_default()
        ...
        if !fetch_tasks.is_empty() {
            ...
            let results =
                futures::future::join_all(fetch_tasks.iter().map(|(_, _, content_part)| {
                    loader.fetch_and_decode_media_part(content_part, media_io_kwargs)
                }))
                .await;

            for ((type_str, slot_idx, _content_part), result) in
