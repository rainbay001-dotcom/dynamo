...
disagg-1p1d-<csp>/           # one overlay per provider — the CSP delta, nothing more
  kustomization.yaml         # references ../_base, sets the name suffix, + a tiny inline benchmark patch (FRONTEND / slug)
  delta.yaml                 # ONE strategic-merge patch: image, args, transport env, RDMA resource, mounts, scheduling
  README.md                  # provider-specific overrides + measured numbers
```

`delta.yaml` reads like a partial `DynamoGraphDeployment` — open it to see everything
that variant changes. Because the CRD has no schema in kustomize, a few list fields
(a worker's `env`, and `args` for the variants that change one) are restated in full
rather than surgically merged; that is the deliberate cost of keeping the patch
plain strategic-merge YAML instead of JSON6902 path edits.
